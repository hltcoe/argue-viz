...
def filter_answers_with_no_references(answers: Dict[str, Answer]) -> Dict[str, Answer]:
    ...
    Args:
        answers: A dictionary of Answer objects keyed by answer text.

    Returns:
        A dictionary of Answer objects that have at least one reference.
    """
...
class NuggetBank(BaseModel):
    ...
    all_nuggets_view: Optional[Dict[str, NuggetQuestion | NuggetClaim]] = Field(
    ...
    def nuggets_as_list(self) -> Optional[List[NuggetQuestion]]:
        if self.nugget_bank is not None:
            assert all(
                isinstance(nugget, NuggetQuestion)
                for nugget in self.nugget_bank.values()
            ), "`nuggets_as_list` only supports NuggetQuestion types."
            return [
                n for n in self.nugget_bank.values() if isinstance(n, NuggetQuestion)
            ]
        else:
            return None
...
def load_nugget_bank_json(source: Union[str, Path, TextIO]) -> NuggetBank:
    ...
    """
    nugget_bank = load_model_json(cls=NuggetBank, source=source)
    nugget_bank.index_nuggets()
    return nugget_bank
...
def filter_empty_citations(nuggetbank: NuggetBank, filter_citations=True) -> NuggetBank:
    ...
    Args:
        nuggetbank: The NuggetBank instance to filter.
        filter_citations: If True, filter out answers with no references. Defaults to True.

    Returns:
        The filtered NuggetBank instance.
    """
